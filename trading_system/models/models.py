...
class TradingRulesDB:
    ...
    def add_rule(
        self, rule_name: str, entry_rule: str, exit_rule: str, close_reverse_rule: str
    ) -> None:
        trading_rule = TradingRule(
            rule_name=rule_name,
            entry_rule=entry_rule,
            exit_rule=exit_rule,
            close_reverse_rule=close_reverse_rule,
        )
...
def calculate_sortino_ratio(
    profits: List[float], risk_free_rate: float = 0.02
) -> float:
    """
...
class SymbolStat(Base, TimestampMixin):
    ...
    def from_barset(cls, symbol: str, barset: BarSet):
        ...
        stats = {
            ...
            "vwap": np.average(
                [bar.c for bar in bars], weights=[bar.v for bar in bars]
            ),
