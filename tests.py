...
class TestStockDataActor:
    ...
    def test_init_valid_parameters(self):
        api = tradeapi.REST(
            "key_id", "secret_key", base_url="https://paper-api.alpaca.markets"
        )
        symbol = "AAPL"
        timeframe = "1D"
    ...
    def test_get_data_invalid_params(self, mocker):
        ...
        mocker.patch.object(
            api,
            "get_barset",
            side_effect=tradeapi.rest.APIError("Invalid symbol or timeframe"),
        )
    ...
    def test_get_data_limit_exceeded(self, mocker):
        ...
        mocker.patch.object(
            api,
            "get_barset",
            side_effect=tradeapi.rest.APIError("Maximum limit exceeded"),
        )
        with pytest.raises(tradeapi.rest.APIError):
...
class TestApplyRules:
    ...
    def test_apply_rules_invalid_input(self, mocker):
        ...
        reverse_rules = [
            Rule(name="rule3", condition="condition3", actions=["action3"])
        ]
        stock_data = [
            StockData(
                symbol="AAPL",
        ...
        positions = [
            Position(
                symbol="AAPL", qty=10, avg_price=100, side="long", time="2022-01-01"
            )
        ]
    ...
    def test_apply_rules_entry_rules_empty(self, mocker):
        ...
        trades, new_positions = apply_rules(
            entry_rules, exit_rules, reverse_rules, stock_data, positions
        )
